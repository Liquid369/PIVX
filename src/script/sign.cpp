...
static bool SignStep(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& scriptPubKey,
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion, SignatureData& sigdata, bool fColdStake)
{
    CScript scriptRet;
    CScriptID scriptID;
    uint160 h160;
...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata, SigVersion sigversion, bool fColdStake, ScriptError* serror)
{
    ...
    solved = SignStep(provider, creator, script, result, whichType, sigversion, sigdata, fColdStake);
    CScript subscript;
    ...
    if (solved && whichType == TX_SCRIPTHASH)
    {
        ...
        solved = solved && SignStep(provider, creator, script, result, whichType, sigversion, sigdata, fColdStake) && whichType != TX_SCRIPTHASH;
        result.emplace_back(subscript.begin(), subscript.end());
    }

    sigdata.scriptSig = PushAll(result);

    sigdata.complete = solved && VerifyScript(sigdata.scriptSig, fromPubKey, STANDARD_SCRIPT_VERIFY_FLAGS, creator.Checker(), sigversion, serror);
...
SignatureData DataFromTransaction(const CMutableTransaction& tx, unsigned int nIn, const CTxOut& txout) {
    ...
    ScriptError* serror;
    Stacks stack(data, sigversion);

    // Get signatures
    MutableTransactionSignatureChecker tx_checker(&tx, nIn, txout.nValue);
    SignatureExtractorChecker extractor_checker(data, tx_checker);
    if (VerifyScript(data.scriptSig, txout.scriptPubKey, STANDARD_SCRIPT_VERIFY_FLAGS,
                     extractor_checker, sigversion, serror)) {
        data.complete = true;
        return data;
    }
...
bool SignSignature(const SigningProvider &provider, const CScript& fromPubKey, CMutableTransaction& txTo, unsigned int nIn, const CAmount& amount, int nHashType, bool fColdStake)
{
    ...
    SigVersion sigversion = SIGVERSION_BASE;
    ScriptError* serror;
    bool ret = ProduceSignature(provider, creator, fromPubKey, sigdata, sigversion, fColdStake,serror);
